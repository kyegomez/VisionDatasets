...
def merge_data(sshfunctions_file, responses_file, output_file):
    ...
    for item in sshfunctions_data:
        ...
        if response_item:
            ...
            item['image_path'] = response_item.get('image_path', 'No image path found')
            item['response'] = response_item.get('response', 'No response found')
...
# Example usage
merge_data('sshfunctions.json', 'responses.json', 'updated_sshfunctions.json')